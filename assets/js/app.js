document.addEventListener("DOMContentLoaded", () => {
  ...
  if (registerForm) {
    registerForm.addEventListener("submit", async (e) => {
      ...
      const nickname = document
        .querySelector("#registerForm input[name='nickname']")
        .value.trim();
      const email = document
        .querySelector("#registerForm input[name='email']")
        .value.trim();
      const password = document.querySelector(
        "#registerForm input[name='password']"
      ).value;
      const confirmPass = document.querySelector(
        "#registerForm input[name='confirmPass']"
      ).value;
      ...
      const response = await fetch(
        "http://localhost/Proyecto/vsgame/api/register.php",
        {
          method: "POST",
          headers: {
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", async (e) => {
      ...
      //fetch de login
      const response = await fetch(
        "http://localhost/Proyecto/vsgame/api/login.php",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  if (logout) {
    logout.addEventListener("click", async () => {
      const response = await fetch(
        "http://localhost/Proyecto/vsgame/api/logout.php",
        {
          method: "POST",
  ...
  async function cargarMazo() {
    const response = await fetch(
      "http://localhost/Proyecto/vsgame/api/start_game.php"
    );
